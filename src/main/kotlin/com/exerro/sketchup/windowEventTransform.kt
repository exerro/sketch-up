...
fun <Model> ConnectedObservableStream<ExtendedWindowEvent>.eventTransformedFold(
    initialModel: Model,
    ...
    val c = fold(WrappedModelState(initialModel, PointerContext.Default)) { (currentModel, context), event -> when (event) {
        ...
        is PointerMoveEvent -> when (context) {
        ...
        is PointerPressedEvent -> when (context) {
        ...
        PointerReleasedEvent -> when (context) {
